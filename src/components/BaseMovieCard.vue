<template>
  <v-hover v-slot="{ isHovering, props }">
    <v-card max-width="350" max-height="650" elevation="3" class="rounded-lg">
      <v-img max-height="500" cover :src="'https://image.tmdb.org/t/p/w300' + movie.poster_path" :alt="movie.title"
        v-bind="props">
        <v-expand-transition>
      ...
      <v-card-actions>
        ...
        <v-spacer></v-spacer>
        <v-btn :class="{ active: movie.inWishlist }" icon @click="
          movie.inWishlist
        ...
        ">
          <svg width="28" height="26" viewBox="0 0 18 16">
            <path
              ...
              :stroke="movie.inWishlist ? 'red' : '#2D2D2D'" :fill="movie.inWishlist ? 'red' : 'transparent'" />
          </svg>
        </v-btn>
      </v-card-actions>
    </v-card>
...
function addMovieToWishlist(movie: Object) {
  const wishlist = {
    ...movie,
    inWishlist: true,
  };
...
function removeMovieInWishlist(wishlistIdMovie: number) {
